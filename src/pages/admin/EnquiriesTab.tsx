...
export default function EnquiriesTab({ token }: { token: string }) {
  ...
  const waLink = (e: any) => {
    ...
    const intlPhone = phone.startsWith("0") ? "254" + phone.slice(1) : phone.startsWith("254") ? phone : "254" + phone;
    const msg = encodeURIComponent(`Hello ${e.name}, thank you for your enquiry about "${e.interest || "our properties"}" on Aeton Homes. We'd love to assist you. When would be a good time to connect?`);
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-end", flexShrink: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {e.phone && (
                      <>
                        <a href={`tel:${e.phone}`} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 12px", background: "rgba(74,222,128,0.12)", border: "1px solid rgba(74,222,128,0.3)", color: "#4ade80", textDecoration: "none", fontSize: "0.7rem", borderRadius: 2, letterSpacing: "0.06em" }}>
                          📞 Call
                        </a>
                        <a href={waLink(e)} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 12px", background: "rgba(37,211,102,0.12)", border: "1px solid rgba(37,211,102,0.3)", color: "#25D366", textDecoration: "none", fontSize: "0.7rem", borderRadius: 2, letterSpacing: "0.06em" }}>
                          💬 WhatsApp
                        </a>
                      </>
                    )}
                    <a href={`mailto:${e.email}`} style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 12px", background: "rgba(201,150,26,0.1)", border: "1px solid rgba(201,150,26,0.2)", color: "#C9961A", textDecoration: "none", fontSize: "0.7rem", borderRadius: 2, letterSpacing: "0.06em" }}>
                      ✉ Email
                    </a>
                  </div>
                  {/* Status change */}
